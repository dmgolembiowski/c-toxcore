...
static void *tox_loop_worker(void *data)
{
    ...
    ck_assert_msg(err == TOX_ERR_LOOP_OK, "tox_loop error: %u", err);
    return nullptr;
}
...
static void test_tox_loop(void)
{
    ...
    ck_assert_msg(err_opts == TOX_ERR_OPTIONS_NEW_OK, "tox_options_new: %u\n", err_opts);
    ...
    ck_assert_msg(err_new == TOX_ERR_NEW_OK, "tox_new: %u\n", err_new);
    ...
    ck_assert_msg(tox_add_tcp_relay(userdata_tcp.tox, TOX_LOCALHOST, TCP_RELAY_PORT, dpk, &error), "Add relay error, %u",
                  error);
    ck_assert_msg(tox_bootstrap(userdata_tcp.tox, TOX_LOCALHOST, 33445, dpk, &error), "Bootstrap error, %u", error);
